...
class PipcConnector(SourceConnector):
    ...
    def fetch_latest(self) -> list[FetchedItem]:
        html = self._http.get_text(
            PIPC_PRESS_LIST, params={"bbsId": "BS074", "mCode": "C020010000"}
        )
        soup = soupify(html)

        out: list[FetchedItem] = []
        for tr in soup.select("table tr"):
            a = tr.select_one('a[href*="selectBoardArticle.do"]')
            if not a:
                continue
            href = a.get("href") or ""
            ...
            if not href or not title:
                continue
            url = href if href.startswith("http") else f"https://www.pipc.go.kr{href}"
